...
- `docs/`：规则、开发指南和项目笔记。
- `local/`：本机配置、日志、工具和参考资料；由 Git 忽略，不属于公共仓库内容。
...
连接参数统一放在本地 `local/config/cyberdog.ps1`；仓库只提供不含真实地址的 `tools/config.example.ps1`。工具仍兼容旧的 `tools/config.ps1`，但会给出迁移提示。
...
个人配置、日志、诊断快照、大型参考文件和个人自动化工具统一放入 `local/`。该目录不会提交到 GitHub；当前电脑的具体说明见 `local/README.md`。
